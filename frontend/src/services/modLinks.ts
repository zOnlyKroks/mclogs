...
export class ModLinkService {
  ...
  private static knownMods: Record<
    string,
    {
      curseforge?: string;
      modrinth?: string;
  ...
  static getModLinks(modName: string): {
    ...
    if (knownMod) {
      return {
        curseforge: knownMod.curseforge
          ? `${this.modSites.curseforge}${knownMod.curseforge}`
          : undefined,
        modrinth: knownMod.modrinth
          ? `${this.modSites.modrinth}${knownMod.modrinth}`
          : undefined,
        github: knownMod.github
          ? `https://github.com/${knownMod.github}`
          : undefined,
      };
    }
  ...
  static getPrimaryLink(modName: string): string {
    ...
    if (
      links.modrinth &&
      !links.modrinth.includes("search") &&
      !links.modrinth.includes("?q=")
    ) {
    ...
    return (
      links.modrinth ||
      `${this.modSites.modrinth}?q=${encodeURIComponent(modName)}`
    );
  }
}
